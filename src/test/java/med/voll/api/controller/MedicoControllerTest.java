...
class MedicoControllerTest {
    ...
    void cadastrarCenario1() throws Exception {
        // Dados inválidos para o médico
        DadosCadastroMedico medicoInvalido = new DadosCadastroMedico("", "", "", "", Especialidade.CARDIOLOGIA, dadosEndereco());  // Exemplo de dados inválidos

        mvc.perform(post("/medicos")
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(dadosCadastroMedicoJson.write(medicoInvalido).getJson())) // Converte o objeto para JSON
                .andExpect(status().isBadRequest()); // Espera que o status seja 400 (BAD_REQUEST)
    }
